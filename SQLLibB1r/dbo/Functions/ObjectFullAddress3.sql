...
CREATE FUNCTION [dbo].[ObjectFullAddress3] 
...
set @name=''
declare @convertstreet nvarchar(100)
...
set  @AddressCursor=cursor
	...
	WITH child_to_parents AS (
	  SELECT convertstreet,SHORTNAME,AOGUID,PARENTGUID,AOLEVEL,streetCODE FROM fias_Moscow.Object
		  WHERE aoGUid = @AOGUID
		  and @gdate between STARTDATE and ENDDATE and ACTSTATUS = 1
	  UNION ALL
	  SELECT o1.convertstreet,o1.SHORTNAME,o1.AOGUID,o1.PARENTGUID,o1.AOLEVEL,o1.streetCODE FROM fias_Moscow.Object o1, child_to_parents
	...
	FROM 
		...
		--ORDER BY aolevel
open @AddressCursor;
fetch next from @AddressCursor into @convertstreet,@shortname,@streetCODE;
if (@WithRoot=0) fetch next from @AddressCursor into @convertstreet,@shortname,@streetCODE
WHILE @@FETCH_STATUS = 0  
BEGIN 
	if (@name ='')
		set @name = @convertstreet + case when @NameFull=0 then '.'  else '' end
	else
		set @name = @name+ @Splitter + 
			case when @streetCODE='0000' then @convertstreet
			else @convertstreet
			end
...
set @name=dbo.deleteDublSpaceNull(@name+@Splitter+ @house) 
